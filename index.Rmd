---
title: "Retrieve details about Winter Olympics 2026 medalists via Wikipedia and Wikidata"
author:
  - name: Giorgio Comai
...
tw_enable_cache()
tw_set_cache_folder(path = fs::path(fs::path_home_r(), "R", "tw_data_olympics2026"))
...
if (fs::file_exists("all_medals_wiki.html")==FALSE) {
  download.file(url = "https://en.wikipedia.org/wiki/List_of_2026_Winter_Olympics_medal_winners",
...
o20_get_medalist_from_cell <- function(row,
  ...
  if (length(links)==0) {
    ...
    links[stringr::str_detect(string = links,
                              pattern = "_at_the_2026_Winter_Olympics",
                              negate = TRUE)] %>% 
      list()
  }
}
...
## Manual fix combined silver

all_medalists_df$delegation_link[all_medalists_df$event_link=="/wiki/Alpine_skiing_at_the_2026_Winter_Olympics_%E2%80%93_Men%27s_team_combined"&all_medalists_df$medalist_link=="/wiki/Marco_Odermatt"] <- "/wiki/Switzerland_at_the_2026_Winter_Olympics"

all_medalists_df$delegation_link[all_medalists_df$event_link=="/wiki/Alpine_skiing_at_the_2026_Winter_Olympics_%E2%80%93_Men%27s_team_combined"&all_medalists_df$medalist_link=="/wiki/Lo%C3%AFc_Meillard"] <- "/wiki/Switzerland_at_the_2026_Winter_Olympics"

all_medalists_df$delegation_link[all_medalists_df$event_link=="/wiki/Ski_jumping_at_the_2026_Winter_Olympics_%E2%80%93_Men%27s_normal_hill_individual"&all_medalists_df$medalist_link=="/wiki/Gregor_Deschwanden"] <- "/wiki/Switzerland_at_the_2026_Winter_Olympics"

all_medalists_df$delegation_link[all_medalists_df$event_link=="/wiki/Alpine_skiing_at_the_2026_Winter_Olympics_%E2%80%93_Women%27s_giant_slalom"&all_medalists_df$medalist_link=="/wiki/Sara_Hector"] <- "/wiki/Sweden_at_the_2026_Winter_Olympics"
...
all_medalists_wd_df <- all_medalists_df %>% 
  dplyr::mutate(medalist_wikidata_id = tw_get_wikipedia_page_qid(medalist_link)[["qid"]],
                event_wikidata_id = tw_get_wikipedia_page_qid(event_link)[["qid"]],
                delegation_wikidata_id = tw_get_wikipedia_page_qid(delegation_link)[["qid"]]) %>% 
...
Are these *all* Olympics medalists? We now have `r all_medalists_wd_df %>% distinct(medalist_wikidata_id) %>% nrow()` and `r all_medalists_wd_df %>% distinct(event_wikidata_id) %>% nrow()` events.
...
```{r all_medalists_with_wd_details_df}
all_medalists_with_wd_details_df <- all_medalists_wd_df %>% 
  mutate(medalist_name = tw_get_label(medalist_wikidata_id),
         place_of_birth_wikidata_id = tw_get_p1(id = medalist_wikidata_id,
                                               p = "P19"), 
         date_of_birth = tw_get_p1(id = medalist_wikidata_id,
                                  p = "P569"), 
         event_sport_wikidata_id = tw_get_p1(id = event_wikidata_id,
                                            p = "P641"),
         event_part_of_wikidata_id = tw_get_p1(id = event_wikidata_id,
                                              p = "P361"),
         event_name = tw_get_label(event_wikidata_id),
         delegation_name = tw_get_label(delegation_wikidata_id), 
         sex_or_gender_wikidata_id = tw_get_p1(id = medalist_wikidata_id,
                                              p = "P21")) %>% 
  dplyr::mutate(medalist_name = dplyr::if_else(
    condition = is.na(medalist_name),
    true = tw_get_label(id = medalist_wikidata_id,
                        language = "mul"),
    false = medalist_name)) %>% 
  mutate(country_medal_wikidata_id = tw_get_p1(id = delegation_wikidata_id,
                                              p = "P17"),
         ...
         place_of_birth_located_in_wikidata_id = tw_get_p1(id = place_of_birth_wikidata_id,
                                                          p = "P131"),
         place_of_birth_coordinates = tw_get_p1(id = place_of_birth_wikidata_id,
                                               p = "P625"),
...
all_medalists_wd_nuts_df <- all_medalists_with_wd_details_df %>% 
  dplyr::left_join(
    all_medalists_with_wd_details_df %>% 
      ...
      sf::st_join(latlon2map::ll_get_nuts_eu(level = 1, year = 2021),
  ...
  dplyr::left_join(
    all_medalists_with_wd_details_df %>% 
      ...
      dplyr::distinct(medalist_wikidata_id, .keep_all = TRUE) %>% 
      sf::st_as_sf(coords = c("lon","lat"), crs = 4326) %>% 
      sf::st_join(latlon2map::ll_get_nuts_eu(level = 2, year = 2021),
                  join = sf::st_intersects) %>% 
      st_drop_geometry() %>% 
      dplyr::transmute(medalist_wikidata_id,
                       nuts2_id = NUTS_ID,
  ...
  dplyr::left_join(
    all_medalists_with_wd_details_df %>% 
      ...
      dplyr::distinct(medalist_wikidata_id, .keep_all = TRUE) %>% 
      sf::st_as_sf(coords = c("lon","lat"), crs = 4326) %>% 
      sf::st_join(latlon2map::ll_get_nuts_eu(level = 3, year = 2021),
                  join = sf::st_intersects) %>% 
      st_drop_geometry() %>% 
      dplyr::transmute(medalist_wikidata_id,
                       nuts3_id = NUTS_ID,
...
population_by_nuts2_2025_file <- fs::path("eurostat",
                                          "population_by_nuts2_2025.csv")

if (fs::file_exists(population_by_nuts2_2025_file)==FALSE) {
  eurostat::get_eurostat(id = "tgs00096", filters = list(age = "TOTAL",
    ...
    readr::write_csv(population_by_nuts2_2025_file)
}

population_by_nuts2 <- readr::read_csv(file = population_by_nuts2_2025_file, show_col_types = FALSE)



population_by_nuts3_2025_file <- fs::path("eurostat",
                                          "population_by_nuts3_2025.csv")

if (fs::file_exists(population_by_nuts3_2025_file)==FALSE) {
  eurostat::get_eurostat(id = "demo_r_pjanaggr3",
    ...
    readr::write_csv(population_by_nuts3_2025_file)
}

population_by_nuts3 <- readr::read_csv(file = population_by_nuts3_2025_file, show_col_types = FALSE)


if (!fs::file_exists(fs::path("eurostat",
                             "gdp_by_nuts3_2023.csv"))) {
...
gdp_by_nuts2_2023_df <- readr::read_csv(fs::path("eurostat",
                                                 "gdp_by_nuts2_2023.csv"),
                                        show_col_types = FALSE)

gdp_by_nuts3_2023_df <- readr::read_csv(fs::path("eurostat",
                                                 "gdp_by_nuts3_2023.csv"),
                                        show_col_types = FALSE)
...
all_medalists_wd_nuts_pop_df <- all_medalists_wd_nuts_df %>% 
  ...
  dplyr::left_join(y = gdp_by_nuts2_2023_df, by = "nuts2_id") %>% 
  dplyr::left_join(y = gdp_by_nuts3_2023_df, by = "nuts3_id") %>% 
...
The share of missing data for NUTS seems high, but this is expected as non-European medalists are mostly not born within a European NUTS region. Only of handful of European medalists do not have information on their place of birth.
...
You can download the dataset [from the repository](https://github.com/giocomai/olympics2026nuts) including this script, or directly [clicking on this link](medalists_all.csv).
...
all_medalists_ll <- all_medalists_df  %>% 
  dplyr::select(lat, lon, medal, medalist_wikidata_id, medalist_name, medalist_link, event_part_of, event_link, delegation_name, delegation_wikidata_id, place_of_birth
                #, img_filename
  ) %>% 
  dplyr::mutate(delegation_icon_link = tw_get_p1(id = delegation_wikidata_id, p = "P18")) %>% 
 ...
 # dplyr::group_by(medalist_wikidata_id) %>% 
  # dplyr::mutate(medal = stringr::str_c(medal),
  #               event_part_of = stringr::str_c(unique(event_part_of))) %>% 
  dplyr::mutate(popup_content = stringr::str_c(
    ...
    "Won ",  "<img src='", medal_icon_link, "' width='16' height='16'> " , medal, " medal in ", 
    "<a href='", event_link, "' target='_blank'>", event_part_of, "</a>",
    " for ", stringr::str_remove(string = delegation_name, pattern = " at the 2026 Winter Olympics"),
    # "<img src='", delegation_icon_link, "' width='16' height='16'> ",
    "</big>"
    #,
...
htmlwidgets::saveWidget(leaflet_medals, file="medalists_map.html",
                        title = "Medalists at the 2026 Winter Olympics by place of birth",
                        selfcontained = TRUE)

leaflet_medals
```

You can see this [interactive map in full screen following this link](https://giocomai.github.io/olympics2026nuts/medalists_map.html).
...
```{r medalists_missing_place_of_birth overwrite_cache, eval = FALSE}
